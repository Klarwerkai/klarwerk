...
- ✅ **Modul `auth`** (`services/auth/`): Fastify-Routen, PBKDF2-Hashing, Sessions, Onboarding + Ersteinrichtung/Status/Nutzerverwaltung (§2.1/§2.2). Deckt FR-AUTH-01…06 (Jira: Done). 15 Tests.
...
- ✅ **Modul `reasoner`** (`services/reasoner/`): austauschbare **async** KI-Schicht (Provider-Interface), **echter Modell-Provider** (`ModelProvider` + Anthropic-HTTP-Client, Schlüssel nur serverseitig via `ANTHROPIC_API_KEY`, `fetch` injizierbar/testbar), Antworten **in echten KOs verankert** (Anti-Halluzination: Quellen/Trust aus den Daten, Modell formuliert nur), deterministischer Fallback ohne Schlüssel **und bei Laufzeitfehlern** (Betrieb bleibt stabil), server-echte Statusanzeige. Deckt FR-RSN-01…06 (FR-RSN-06 backend erfüllt). 15 Tests. Läuft ohne Key (deterministisch); mit Key automatisch echtes Modell.
...
- ✅ **Modul `ask`** (`services/ask/`): Frage→begründete Antwort über den Reasoner, semantische Auswahl, ehrliche Verweigerung → Wissenslücke, „Hat geholfen" (Trust + Audit-Eintrag), Lücken-Verwaltung (zuweisen/schließen/bestätigt löschen). Deckt FR-ASK-01…06 (Jira: Done) — Antwort trägt jetzt die **Belegstelle/Snippet** der Quelle (FR-ASK-06). Plus Ask-Telemetrie fürs Wirkungs-Dashboard. Bindet reasoner + knowledge-object + audit zusammen.
- ✅ **Modul `library-analytics`** (`services/library-analytics/`): Volltextsuche+Filter, Export JSON/MediaWiki, Import ohne Duplikate, Bus-Faktor (Einzelquellen), Wissensgraph aus gemeinsamen Tags, Analytics nach Status/Art/Kategorie. Deckt FR-LIB-01/03/04 + FR-ANA-01 (Jira: Done). **FR-ANA-02 Wirkungs-Dashboard** (`GET /api/analytics/impact`) und **FR-LIB-02 PDF-Export** (`?format=html` → druckfertiges HTML, PDF via Browser-Druck) — erledigt.
...
- ✅ **Composition-Root `app`** (`services/app/`): verdrahtet **alle** Fachmodule zu **einer** Fastify-App (modularer Monolith) und stellt die **vollständige HTTP-API** bereit (Technischer Anhang §2.1–2.5, ~48 Pfade): Auth/Status/Setup, Nutzerverwaltung, Wissensobjekte inkl. Mutations-Dispatcher `PUT /api/kos/:id` (rate/assign/revise/category/tags/conflict/resolve/transfer-author/revalidate), Validierungs-Board + Übersicht, Konflikte (Liste/Eskalation/Zweitmeinung), Entwürfe + Promote→KO, Fragen/Lücken + „Hat geholfen", Bibliothek/Export/Import/Analytics/Bus-Faktor/Graph, Lebenszyklus/Lernpfade, Audit-Log, Reasoner (structure|ask), i18n. Gemeinsamer `http.ts`-Baustein: Auth- + RBAC-Guard + einheitliches Fehler-Mapping (serverseitig erzwungen, FR-RBAC-04). 5 Tests (End-to-end über alle Module).
- ✅ **Audit-Verdrahtung (FR-AUD-01: Done):** `AuditService` deckt alle §12.3-Aktionen mit Feature ab — Erfassen, Validieren/Ablehnen (Bewerten), Re-Validierung, „Hat geholfen", Nutzerverwaltung, Login/Logout, Konflikt/Eskalation/Auflösung, Zuweisung, Kategorieänderung, Autor-Übergabe, Löschen, Export/Import — je mit Actor (wer/was/wann), append-only Hash-Kette über alle Module. (Nur „Kommentieren" offen, da kein Feature.) Macht auch **FR-RBAC-02 Done**.
- **Alle Gates grün** (99 Unit-Tests + 1 Integrationstest): Build · Lint (Biome) · Architektur (dependency-cruiser, 104 Module, 322 Abhängigkeiten inkl. Typ-Importe, Modulgrenzen nur über index) · Tests. **HTTP-API vollständig** (§2.1–2.5) + **echter Reasoner-Provider** + **vollständige Postgres-Persistenz** + **E-Mail (SMTP)** + **SSO/OIDC** — alle Pflichtenheft-FRs sind backendseitig abgedeckt; offen bleibt nur das Frontend.
...
- **Persistenz: Postgres-Adapter vollständig (alle Module).** `PgUserRepo`/`PgSessionRepo`, `PgKoRepo` (JSONB), `PgAuditRepo`, `PgDraftRepo`, `PgGapRepo`, `PgRatingRepo`/`PgAssignmentRepo` (zusammengesetzte Schlüssel, Upsert), `PgConflictRepo`, `PgLifecycleRepo` (Kopplungen/Re-Validierung/Lernpfade über mehrere Tabellen) — alle hinter denselben Repo-Interfaces, je Modul eigene DDL (Datenhoheit). Shared Pool + `migrate()` (`services/app/src/db.ts`); `buildPgServices(pool)` = exakt dieselbe App gegen echte DB (ein `assembleServices`-Helfer, In-Memory vs. Postgres = nur der Repo-Satz). **Integrationstest** (Testcontainers, echtes Postgres) prüft Register→Login→KO→Audit **und** Entwurf→KO, Bewertung, Lernpfad — persistent über App-Instanzen hinweg. Getrennt vom schnellen Gate: `npm run test:integration` (Docker). In-Memory bleibt für Tests/Dev ohne DB. Auch erledigt: **Passwort-Self-Service** (`POST /api/auth/password`, altes Passwort nötig, alte Sitzungen verfallen) und **Cookie-Härtung** (Secure-Flag via `COOKIE_SECURE=true` in Produktion). Auch erledigt: **E-Mail-Infrastruktur** (Modul `notifications`: anbieteragnostischer Mailer, SMTP-Adapter via `nodemailer`, `ConsoleMailer`-Fallback, `SMTP_*`-env), **FR-VAL-07** (Zuweisung → E-Mail), **FR-AUTH-08** (Passwort-Reset per E-Mail: persistenter Token-Store In-Memory + Pg, Routen `/forgot`+`/reset`) und **FR-AUTH-07** (generisches OIDC: `jose`-Verifier mit Discovery/JWKS, `POST /api/auth/oidc`, optionale Auto-Provisionierung, `OIDC_*`-env). **Noch offen:** nur das Frontend.
- **Deploy-ready:** `Dockerfile` (Backend als Container, identisch Hetzner/On-Prem/Cloud), `docker-compose.prod.yml` (App + Postgres), Produktions-Start `services/app/src/server.ts` (`npm start`; mit `DATABASE_URL` → Postgres + Migration, sonst In-Memory), vollständige `.env.example`. **Hosting gewählt:** Hetzner CAX21 (Falkenstein, DE) + Coolify + Cloudflare-DNS — Schritt-für-Schritt in `docs/operations/deploy-hetzner.md`. **Die Coolify-Betriebseigenschaften (TLS, Backup, Rollback, Webhook, Proxy-Zugangsschutz) sind dort als unbestätigt gekennzeichnet** — die Hosting-Entscheidung ist belegt, ihr Betriebszustand nicht.
- **Neue Befehle/Deps:** `npm install` (zieht `pg` + `@types/pg`), `npm run test:integration` (Postgres via Docker). Auf dem Mac einmal ausführen, um die Persistenz live zu verifizieren.
