...
- ✅ **Audit-Verdrahtung (FR-AUD-01: Done):** `AuditService` deckt alle §12.3-Aktionen mit Feature ab — Erfassen, Validieren/Ablehnen (Bewerten), Re-Validierung, „Hat geholfen", Nutzerverwaltung, Login/Logout, Konflikt/Eskalation/Auflösung, Zuweisung, Kategorieänderung, Autor-Übergabe, Export/Import — je mit Actor (wer/was/wann), append-only Hash-Kette über alle Module. (Nur „Kommentieren" offen, da kein Feature.) Macht auch **FR-RBAC-02 Done**.
- **Alle Gates grün** (77 Tests gesamt): Build · Lint (Biome) · Architektur (dependency-cruiser, 68 Module, 184 Abhängigkeiten inkl. Typ-Importe, Modulgrenzen nur über index) · Tests.
