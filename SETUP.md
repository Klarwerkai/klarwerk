...
- ✅ **Composition-Root `app`** (`services/app/`): verdrahtet alle Module zu **einer** Fastify-App (modularer Monolith). `buildApp()` + `server.ts`-Einstiegspunkt; Endpunkte `/health`, `/api/reasoner/status`, Auth-Routen, geschützte `/api/kos`. End-to-end-Test: Registrierung → Login → rechtegeprüfte KO-Liste über vier Module. 2 Tests.
- **Alle Gates grün** (45 Tests gesamt): Build · Lint (Biome) · Architektur (dependency-cruiser, 40 Module, 100 Abhängigkeiten inkl. Typ-Importe, Modulgrenzen nur über index) · Tests.
