...
- **Alle Gates grün** (43 Tests gesamt): Build · Lint (Biome) · Architektur (dependency-cruiser, 36 Module, 90 Abhängigkeiten inkl. Typ-Importe, Modulgrenzen nur über index) · Tests.
- **Backend-Vertikale durchgängig:** Entwurf (`capture`) → `toKoInput` → KO anlegen (`knowledge-object`) → bewerten/validieren (`validation`) → Status „validiert". End-to-end über Modulgrenzen testbar. `reasoner` als Querschnitt für structure/ask/conflicts bereit.
