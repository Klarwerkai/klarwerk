...
- ✅ **Composition-Root `app`** (`services/app/`): verdrahtet alle Fachmodule zu **einer** Fastify-App (modularer Monolith). `buildApp()` + `server.ts`-Einstiegspunkt; Endpunkte `/health`, `/api/reasoner/status`, Auth-Routen, geschützte `/api/kos`. End-to-end-Test über vier Module. 2 Tests.
- **Alle Gates grün** (50 Tests gesamt): Build · Lint (Biome) · Architektur (dependency-cruiser, 46 Module, 115 Abhängigkeiten inkl. Typ-Importe, Modulgrenzen nur über index) · Tests.
