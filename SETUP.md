...
- 🟡 **Audit-Verdrahtung (FR-AUD-01, läuft):** `AuditService` verdrahtet in **auth** (Admin-Aktionen → FR-RBAC-02 **Done**), **knowledge-object** (Anlegen/Überarbeiten) und **validation** (Bewerten). Append-only-Kette über alle Module intakt.
  - **Laut Funktionsbeschreibung §12.3 noch zu verdrahten** für FR-AUD-01 „Done": Login/Logout (auth), Konflikt/Eskalation/Auflösung (conflicts), Zuweisung (validation.assign), Kategorieänderung (ko.updateCategory), Autor-Übergabe (lifecycle), Export/Import (library-analytics). (Kommentieren = noch kein Feature.)
- **Alle Gates grün** (74 Tests gesamt): Build · Lint (Biome) · Architektur (dependency-cruiser, 68 Module, 180 Abhängigkeiten inkl. Typ-Importe, Modulgrenzen nur über index) · Tests.
