...
describe.each(["bearer", "cookie"] as const)(
  "M3c · Quellenfund im importierten Volltext · %s",
  (anmeldeweg) => {
...
it("C0 · Login liefert ausschließlich das vom Server gesetzte Cookie", async () => {
  const { app } = await aufbau();
  try {
    const headers = await loginCookie(app, "autor@m3c.test", "geheim12345");
    expect(Object.keys(headers)).toEqual(["cookie"]);
    expect(headers.cookie).toMatch(/^[^=;]+=[^;]+$/);
    expect((await pruefe(app, headers, PASSAGE)).quellenfund.gelaufen).toBe(true);
  } finally {
    await app.close();
  }
});
